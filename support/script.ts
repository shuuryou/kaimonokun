...
class kaimonokun {
    ...
    constructor(authline: string) {
        ...
        if (document.readyState != 'loading') this.initialize();
        else
    ...
    private sortItems() {
        if (this.m_CurrentListId == null) return;
    ...
    private loadLists(sneaky: boolean) {
        ...
        if (!sneaky) this.UIMode = UIModes.Loading;
        ...
        fetch(kaimonokun.BACKEND_LOCATION, requestOptions)
            .then(async (response) => {
                ...
                const items: HTMLElement = document.querySelector(
                    '#lists'
                )! as HTMLElement;
                ...
                for (var id in data)
                    if (data.hasOwnProperty(id))
                        newitems.append(
                            this.makeList(id, data[id as keyof typeof data] as string)
                        );

                while (items.lastChild) items.removeChild(items.lastChild);

                items.appendChild(newitems);

                if (!sneaky) this.UIMode = UIModes.Lists;
    ...
    private loadItems(sneaky: boolean, listId: string) {
        ...
        fetch(kaimonokun.BACKEND_LOCATION, requestOptions)
            .then(async (response) => {
                ...
                for (var id in data)
                    if (data.hasOwnProperty(id))
                        newitems.appendChild(
                            this.makeItem(
                                id,
                                data[id as keyof typeof data]['item'],
                                data[id as keyof typeof data]['checked']
                            )
                        );
                ...
                if (!sneaky) this.UIMode = UIModes.Items;
    ...
    private createEventSource() {
        if (this.m_EventSource != null) return;

        if (document.visibilityState != 'visible') return;

        let watch;

        if ((this.UIMode & UIModes.Lists) === UIModes.Lists) watch = 'lists';
        else if ((this.UIMode & UIModes.Items) === UIModes.Items)
            watch = this.m_CurrentListId;
        else throw 'Invalid mode.';

        this.m_EventSource = new EventSource(
            kaimonokun.EVENTSOURCE_LOCATION +
            '?watch=' +
            watch +
            '&auth=' +
            this.m_AuthLine
        );
        this.m_EventSource.addEventListener('message', (ev) =>
            this.eventSourceOnMessageImpl(ev)
        );
        this.m_EventSource.addEventListener('error', () =>
            this.eventSourceOnErrorImpl()
        );
    }
    ...
    private eventSourceOnErrorImpl() {
        ...
        if (this.m_EventSource.readyState == EventSource.CONNECTING) return;

        if (this.m_EventSource.readyState == EventSource.CLOSED) {
            this.showModal(
                T_EVENTSOURCE_ERROR_TITLE,
                T_EVENTSOURCE_ERROR_GENERIC_TEXT
            );
            return;
        }
    }
    ...
    private eventSourceOnMessageImpl(e: MessageEvent) {
        ...
        if (this.m_CurrentDigest === digest) {
            console.log(
                'EventSource: Remote digest %s is the same as ours, so ignore.',
                digest
            );
            return;
        }
        ...
        console.log(
            'EventSource: Remote digest is %s and ours is %s.',
            digest,
            this.m_CurrentDigest
        );
    ...
    private makeList(listId: string, listName: string) {
        const tpl: HTMLTemplateElement = document.getElementById(
            'tpl-list'
        ) as HTMLTemplateElement;
        const content: HTMLElement = (tpl.content.cloneNode(true) as HTMLElement)
    ...
    private handleSave(evt: Event) {
            ...
            if ((this.UIMode & UIModes.Lists) === UIModes.Lists) {
                ...
                fetch(kaimonokun.BACKEND_LOCATION, requestOptions)
                    ...
                    .catch((error) => {
                        this.showModal(
                            T_AJAX_ERROR_TITLE,
                            T_AJAX_ERROR_GENERIC_TEXT + error
                        );
                        return;
                    });
            } else if ((this.UIMode & UIModes.Items) === UIModes.Items) {
        ...
        if (!this.m_UpdatePending) return;

        if ((this.UIMode & UIModes.Lists) === UIModes.Lists) this.loadLists(true);
        else if ((this.UIMode & UIModes.Items) === UIModes.Items)
            this.loadItems(true, this.m_CurrentListId!);
        else throw 'Invalid mode.';
    }
    ...
    private handleDragEnd() {
        ...
        document
            ...
            .forEach(function (el: Element) {
                new_order += (el as HTMLElement).dataset['itemid'] + '/';
            });
    ...
    private handleDragEnter(evt: DragEvent) {
        ...
        if (target_el.nodeType != Node.ELEMENT_NODE) return;

        // Can only enter item contaienrs, not their children
        if (!target_el.classList.contains('item')) return;
    ...
    private handleTouch(evt: TouchEvent) {
        ...
        if (!el) return; // No item container, WTF?
    ...
    private handleTouchMove(evt: TouchEvent) {
        ...
        if (Date.now() - this.m_TouchTime < kaimonokun.TOUCH_MIN_TIME_MS) return;
    ...
    private set UIMode(mode: UIModes) {
        ...
        document.querySelector(
            'div.head a.backbutton'
        )!.parentElement!.style.display = isItems ? 'block' : 'none';
        document.querySelector(
            'div.head a.sortbutton'
        )!.parentElement!.style.display = isItems ? 'block' : 'none';
