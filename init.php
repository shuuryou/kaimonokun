...
function __($Constant, array $Arguments = array() , $Format = true)
{
    ...
    if ($Format) return nl2br(Fix(vsprintf(constant($Constant) , $Arguments)));
    else return vsprintf(constant($Constant) , $Arguments);
}
...
{
    if (!file_exists($file)) return '0000000000';

    clearstatcache();
    return substr(sha1(filemtime($file)) , 0, 10);
}